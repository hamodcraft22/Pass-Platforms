...
export default function RevisionsTableRow({bookingID, subject, date, startTime, endTime, status, online, viewType, userType})
{
    ...
    async function deleteRevision()
    {
        try
        {
            ...
            await fetch(`https://zift.ddnsfree.com:5679/api/revision/${bookingID}`, requestOptions)
                .then(response =>
                {
                    if (response.status === 201 || response.status === 200)
                    {
    ...
    async function deregisterRevision()
    {
        try
        {
            ...
            await fetch(`https://zift.ddnsfree.com:5679/api/revision/${bookingID}/member`, requestOptions)
                .then(response =>
                {
                    if (response.status === 201 || response.status === 200)
                    {
